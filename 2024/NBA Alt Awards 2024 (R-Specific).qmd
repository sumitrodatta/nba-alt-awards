...
```{r clutch_ft_worst}
free_throws=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"Free Throw")|
           str_detect(VISITORDESCRIPTION,"Free Throw"))
...
clutch_differential=inner_join(clutch_free_throws,non_clutch_free_throws) %>% 
  mutate(differential=non_clutch_ft_percent-clutch_ft_percent) %>% 
...
```{r no_fly_zone}
misses=pbp2024 %>% 
  filter(str_starts(HOMEDESCRIPTION,"MISS")|str_starts(VISITORDESCRIPTION,"MISS"))
missed_dunks=misses %>% 
  filter(str_detect(HOMEDESCRIPTION,"Dunk")|str_detect(VISITORDESCRIPTION,"Dunk"))
blocked_dunks=missed_dunks %>% 
  filter(str_detect(HOMEDESCRIPTION,"BLOCK")|str_detect(VISITORDESCRIPTION,"BLOCK"))
#as the blocker
...
blocks=pbp2024_w_next_play %>% 
  filter(str_detect(HOMEDESCRIPTION,"BLOCK")|
           str_detect(VISITORDESCRIPTION,"BLOCK")) %>%
  mutate(home_or_visitor_block=if_else(
    str_detect(HOMEDESCRIPTION,"BLOCK"),"home","visitor"))

team_rebounds=pbp2024_w_next_play %>% 
  filter(str_ends(HOMEDESCRIPTION,"Rebound")|
           str_ends(VISITORDESCRIPTION,"Rebound"))
...
shot_after_own_rebound_miss=pbp2024_w_before_after %>% 
  filter(str_detect(HOMEDESCRIPTION,"REBOUND")|
           str_detect(VISITORDESCRIPTION,"REBOUND")) %>%
...
#get alley oops
alley_oops=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"Alley")|
           str_detect(VISITORDESCRIPTION,"Alley"))
#remove missed alley oop attempts
made_alley_oops=alley_oops %>% 
  filter(str_starts(HOMEDESCRIPTION,"MISS",negate=TRUE)| 
           str_starts(VISITORDESCRIPTION,"MISS",negate=TRUE))
...
```{r two_plus_dunks}
dunks=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"Dunk")|
           str_detect(VISITORDESCRIPTION,"Dunk"))
missed_dunks=dunks %>%
  filter(str_detect(HOMEDESCRIPTION,"MISS")| 
...
```{r bowling_ball}
charges=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"Charge")|
           str_detect(VISITORDESCRIPTION,"Charge"))
...
```{r goaltends}
goaltends=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"Goaltending")|
           str_detect(VISITORDESCRIPTION,"Goaltending"))
...
kicked_balls=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"Kick")|
           str_detect(VISITORDESCRIPTION,"Kick"))
...
def_3_seconds=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"3 Second")|
           str_detect(VISITORDESCRIPTION,"3 Second"))
...
third_free_throws=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"Free Throw 3 of 3")|
           str_detect(VISITORDESCRIPTION,"Free Throw 3 of 3"))

shooting_fouls=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"S.FOUL")|
           str_detect(VISITORDESCRIPTION,"S.FOUL"))

fouls_on_missed_threes=inner_join(shooting_fouls,third_free_throws,
                                  by=c('GAME_ID','PERIOD','PCTIMESTRING'))

three_pointers=pbp2024 %>% 
  filter(str_detect(HOMEDESCRIPTION,"3PT")|
           str_detect(VISITORDESCRIPTION,"3PT"))

made_threes=three_pointers %>% 
  filter(str_starts(HOMEDESCRIPTION,"MISS",negate=TRUE)|
           str_detect(VISITORDESCRIPTION,"MISS",negate=TRUE))
...
best_worst_starter_candidates=advanced %>% left_join(.,per_game) %>%
  ...
  arrange(desc(mp_per_game)) %>% group_by(tm,season) %>% 
  slice(1:5) %>% slice_min(vorp) %>% ungroup() 
...
```{r master_baiter_scrape,results='hide',cache=TRUE}
...
Sys.sleep(10)
a$clickElement()
b<-remDr$findElement(using="xpath",value=paste0("//*/option[@value = '491']"))
b$clickElement()
...
rate_based_3pt_fouls_drawn=left_join(
  ...
  totals %>% select(seas_id:tm,x3p,x3pa) %>% 
    mutate(name_to_join=case_when(
      ...
      str_detect(player,"KJ Martin")~"Kenyon Martin Jr.",
...
salary_performance %>% filter(experience > 4,salary>1017781) %>% 
  arrange(desc(vorp_per_million)) %>% select(player,salary,vorp,vorp_per_million) %>% gt() %>% 
...
salary_performance %>% filter(experience > 4,percent_of_cap>0.05) %>% 
  arrange(desc(vorp_per_million)) %>% select(player,salary,vorp,vorp_per_million,percent_of_cap) %>% gt() %>% 
  fmt_currency(columns=salary,use_subunits = FALSE) %>%
  fmt_number(columns=vorp_per_million,decimals = 4) %>%
...
pos_percents_w_heights=play_by_play %>% 
  ...
  mutate(position=case_when(
    full_in_ht>=84~"big_percent", #evan mobley classified as big_wing
...
pos_vorp_leader_filter<-function(position,conf){
  final_df<-pos_vorps_w_rotations %>% filter(position_2==position,conference==conf) %>% slice_max(pos_vorp_wo_leader,n=5) %>% select(tm,rotation,pos_vorp_wo_leader)
}
```
