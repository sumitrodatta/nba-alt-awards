...
made_dunks %>% group_by(GAME_ID,PLAYER1_NAME) %>% summarize(dunks_in_game=n()) %>%
  ...
  summarize(games_w_2plus_dunks=n()) %>% slice_max(games_w_2plus_dunks,n=10)
```
...
write_csv(pos_percents_w_heights %>% 
            group_by(tm,position_2) %>% 
            mutate(leader=if_else(max(vorp)==vorp,TRUE,FALSE)) %>% ungroup(),
