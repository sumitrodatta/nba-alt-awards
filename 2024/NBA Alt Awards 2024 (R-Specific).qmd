...
coaches_challenge_by_day=scrape(session) %>% 
  ...
  rename(links=".") %>%
  distinct(links) %>%
  filter(str_detect(links,"Coachs-Challenge")) %>% slice(1) %>% pull()

challenge_tables=extract_tables(file=coaches_challenge_by_day)

all_challenges=challenge_tables[[2]] %>% as_tibble(.name_repair = "minimal") %>% row_to_names(2) %>% clean_names()

for (x in 3:length(challenge_tables)){
